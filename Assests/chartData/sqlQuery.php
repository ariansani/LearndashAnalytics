...
//THIS QUERY SHOWS ALL USERS IN SYSTEM
...
/*{$wpdb->prefix}posts.id AS 'Course ID'*/
$query = "SELECT {$wpdb->prefix}posts.post_title AS 'Course Name', {$wpdb->prefix}learndash_user_activity.course_id AS 'Course ID',wp_users.ID  AS 'User ID', wp_users.display_name as 'Display Name'
...
WHERE {$wpdb->prefix}learndash_user_activity.activity_type = 'access' AND {$wpdb->prefix}posts.post_type='sfwd-courses' AND {$wpdb->prefix}posts.post_status='publish' AND wp_users.ID IS NOT NULL AND {$wpdb->prefix}learndash_user_activity.activity_status IS NULL AND {$wpdb->prefix}learndash_user_activity.activity_started != 0";

/*$query = "SELECT `{$wpdb->prefix}posts`.id AS 'Course ID', `{$wpdb->prefix}posts`.post_title AS 'Course Name', `{$wpdb->prefix}learndash_user_activity`.course_id AS 'Course ID',`{$prefix}users`.ID  AS 'User ID', `{$prefix}users`.display_name as 'Display Name'
FROM wp_11_learndash_user_activity 
INNER JOIN `{$wpdb->prefix}posts` ON {$wpdb->prefix}posts`.id = `{$wpdb->prefix}_learndash_user_activity`.course_id
INNER JOIN `{$prefix}users` ON {$prefix}users`.ID = `{$wpdb->prefix}learndash_user_activity`.user_id
